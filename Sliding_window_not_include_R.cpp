...
void solve() {
    // index l is start pointer of the window, and index r is end pointer 
    // ===> so index r is included in the valid range

    // Always keep [l,r] valid by checking if r + 1 is valid and can be included
    // + if valid => r+1 can be added in range, then increase r for next check
    // + if [l,r+1] not valid => gradually increase l to get valid range

    // Note: maximum of r always nums.size() - 1, so we need some how break the loop
    // Either by first condition: while (r < nums.size() - 1)
    // Or specify condition if (r == nums.size() - 1) break; in the while loop

    int l = 0, r = 0;   // r = 0 mean accept index 0, r = -1 mean not accept index 0
    while (r < nums.size()) {
        // Condition to increase r
        while (r + 1 < nums.size() && nums[r + 1] <= somethings) {  
            r++;
        }

        // We have valid window here, do something
        // Do something

        // Condition to increase l
        while (l < nums.size() && nums[l] == something) l++;
